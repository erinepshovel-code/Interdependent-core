...
from prime_core import pcna, pcta, ptca
from prime_core.pcna import PCNANetwork
from prime_core.pcta import PCTACircle
from prime_core.ptca import (
    CORE_SEEDS,
...
def test_top_level_subpackages_exposed():
    ...
    assert prime_core.pcna is pcna
    assert prime_core.pcta is pcta
    assert prime_core.ptca is ptca
    # pcea + theta bridge are part of the quartet; edcm lives elsewhere.
    assert hasattr(prime_core, "pcea")
    assert hasattr(prime_core, "theta")
    assert prime_core.PRIME_CORE_SIZE == 53
